...
# Read in the data
df.hatch <- read_sav(here("df_scaled.sav"))
```
...
# BDI Density
f1 <- ggplot(data = df.long, aes(x = BDI)) +
  geom_density() +
...
# Log-BDI Density
f2 <- ggplot(data = df_scaled, aes(x = log_BDI)) +
  geom_density() +
  facet_grid(parent~time) +
...
ggplot(data = distinct(na.omit(df.long), CoupID, .keep_all = T), 
