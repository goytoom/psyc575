...
# Read in the data
df.hatch <- read_sav("/Users/Lizzie/Documents/USC/Coursework/21F/575/575 R Scripts/final_project/HATCH 09.29.21.sav")
```
...
## Random Slope Tests
```{r message=FALSE}
m01 <- lmer(log_BDI ~ 1 + (time|PersonID), data=df_scaled)
m02 <- lmer(log_BDI ~ 1 + (time|CoupID), data=df_scaled)
m03 <- lmer(log_BDI ~ 1 + (time|PersonID) + (time|CoupID), data = df_scaled) 
ranova(m01) #sig
ranova(m02) #nonsig
ranova(m03) #person=nonsig; couple=sig
```

## Basic Models
```{r}
# Raw BDI
m1 <- glmmTMB(BDI ~ beq_cmc + time*DelMeth*beq_cm + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m1)

# Log-Transformed BDI
m2 <- glmmTMB(log_BDI ~ beq_cmc + time*DelMeth*beq_cm + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m2)

# Log-Transformed BDI without 2-way interaction
m3 <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m3)

# EPDS
m3.5 <- glmmTMB(log_EPDS ~ beq_cmc + time + DelMeth*beq_cm + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m3.5)
```

1. delivery method predicts BDI 
2. couple-level beq predicts BDI
3. BDI is impacted by time-since-birth
4. del meth and couple-level beq work together to predict BDI, and that interaction does NOT change over time?
...
```{r}
# Gender
m4 <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm + parent + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m4)

## Mom-Only
df_mom <- filter(df_scaled, parent == "mom")
m_mom <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm  + (time|PersonID) + (1|CoupID), data = df_mom)
summary(m_mom)

## Dad-Only
df_dad <- filter(df_scaled, parent == "dad")
m_dad <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm  + (time|PersonID) + (1|CoupID), data = df_dad)
summary(m_dad)

# Gender and Prenatal Depression
m5 <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm + parent + pnBDI + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m5)

# Gender, pnBDI, parent-level demographics
m6 <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm + parent + pnBDI + ethnicity + age + education + (time|PersonID) + (1|CoupID), data = df_scaled)
summary(m6)
