...
p1 <- ggplot(data = df.long, aes(x = BDI, fill=parent)) +
  geom_density(alpha=0.6, show.legend = FALSE) +
  facet_grid(.~time, scales = "free", space='free') +
  labs(x = "Depression (BDI)", y = "")

p2 <- ggplot(data = df_scaled, aes(x = log_BDI, fill=parent)) +
  geom_density(alpha=0.6) +
  facet_grid(.~time) +
  labs(x = "Log-Transformed Depression (BDI)", y = "")

p3 <- ggplot(data = df.long, aes(x = EPDS, fill = parent)) +
  geom_density(alpha = 0.5) +
  facet_grid(.~time) +
  labs(x = "PP Depression (EPDS)", y = "")

p4 <- ggplot(data = df.long, aes(x = beq, fill = parent)) + 
  geom_density(alpha = 0.5) + 
...
# Log-Transformed BDI without 3way time interaction, does not converge with random slope, so take out!
m3 <- glmmTMB(log_BDI ~ beq_cmc + time*DelMeth + time*beq_cm + DelMeth*beq_cm + (time|CoupID/PersonID), data = df_scaled, control = glmmTMBControl(optimizer = optim, optArgs = list(method="BFGS")))
summary(m3)

# Log-Transformed BDI without time interaction, does not converge with random slope, so take out!
m3.5 <- glmmTMB(log_BDI ~ beq_cmc + time + DelMeth*beq_cm + (1|CoupID/PersonID), data = df_scaled, control = glmmTMBControl(optimizer = optim, optArgs = list(method="BFGS")))
summary(m3.5)

# EPDS
m4 <- glmmTMB(log_EPDS ~ beq_cmc + time*DelMeth + time*beq_cm + DelMeth*beq_cm + (time|CoupID) + (1|CoupID:PersonID), data = df_scaled, control = glmmTMBControl(optimizer = optim, optArgs = list(method="BFGS")))
summary(m4)
```
...
```{r}
# Gender -> significant
m4 <- glmmTMB(log_BDI ~ beq_cmc + time*beq_cm + time*DelMeth + DelMeth*beq_cm + parent + (time|CoupID/PersonID), data = df_scaled)
summary(m4) 
...
```{r}
# m4 including baby demographics
m5_robust <- brm(
  log_BDI ~ beq_cmc + time*DelMeth + beq_cm*DelMeth + time*beq_cm + parent + pnBDI + GestationAge + BabySex + (time|CoupID/PersonID),
  # Assume the errors follow a t distribution
  family = student(),
  data = df_scaled,
  chains = 2,
...
```{r}
# m4 including parent demographics
m6_robust <- brm(
  log_BDI ~ beq_cmc + time*DelMeth + time*beq_cm + DelMeth*beq_cm + parent + pnBDI + ethnicity + age + education + (time|CoupID/PersonID),
